...
class ContentProvider extends ChangeNotifier {
  ...
  Future<void> _fetchTruthDare() async {
    final snap = await _db.collection('truth_dare_tasks').where('active', isEqualTo: true).get();
    truths.clear();
    dares.clear();
  ...
  Future<void> _fetchFourCards() async {
    final snap = await _db.collection('four_cards_tasks').where('active', isEqualTo: true).get();
    fourCardsData.clear();
    ...
    for (var doc in snap.docs) {
      ...
      if (!fourCardsData.containsKey(catId)) {
        fourCardsData[catId] = [];
      }
  ...
  Future<void> _fetchMaze() async {
    ...
    for (var doc in snap.docs) {
      int cellId = int.parse(doc.id); // Document ID is the cell number
